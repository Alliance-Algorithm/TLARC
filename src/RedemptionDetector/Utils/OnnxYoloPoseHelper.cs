...
public class OnnxYoloHelper
{
  ...
  private DenseTensor<float> PreprocessImage(Mat image)
  {
    ...
    for (int y = 0; y < targetHeight; y++)
    {
      ...
      {
        input[0 * stride + y * targetWidth + x] =
          img.Data[y, x, 1] / 255.0f > thransholdInOne ? 1 : 0; // B通道
        input[1 * stride + y * targetWidth + x] =
          img.Data[y, x, 1] / 255.0f > thransholdInOne ? 1 : 0; // G通道
        input[2 * stride + y * targetWidth + x] =
          img.Data[y, x, 1] / 255.0f > thransholdInOne ? 1 : 0; // R通道
      }
    }
  ...
  private Dictionary<
    int,
    (Rectangle box, List<Point> keypoints, float confidence, float keypointConfidenceSum)
  > PostprocessOutput(Tensor<float> outputTensor, float originalWidth, float originalHeight)
  {
    ...
    for (int i = 0; i < numDetections; i++)
    {
      ...
      results[classId] = results.ContainsKey(classId)
        ? (
          results[classId].Item3 > confidence
            ? results[classId]
            : (box, keypoints, confidence, keypointConfidenceSum)
        )
        : (box, keypoints, confidence, keypointConfidenceSum);
    }
  ...
  public void DrawDetections(
    ...
    Dictionary<
      int,
      (Rectangle box, List<PointF> keypoints, float confidence, float keypointConfidenceSum)
    > detections
  )
  {
