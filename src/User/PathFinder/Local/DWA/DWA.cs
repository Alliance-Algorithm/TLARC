...
namespace AllianceDM.Nav
{
    ...
    public class DWA(uint uuid, uint[] revid, string[] args) : LocalPathFinder(uuid, revid, args)
    {
        ...
        public override void Update()
        {
            ...
            else
            {
                ...
                foreach (var v in model.Output.Sample)
                {
                    ...
                    if (t > maxValue)
                    {
                        Dir = Rotate(v, sentry.Output.angle) + model.Output.Current;
        ...
        {
            // float angle = Vector2.Dot(Rotate(predict, sentry.Output.angle), model.Output.Current);
            float angle = Vector2.Dot(Rotate(predict, sentry.Output.angle) + model.Output.Current, model.Output.Current);
            if (currentSpeed == 0)
                angle = 0;
            else
            {
                // angle = angle / (Rotate(predict, sentry.Output.angle)).Length() / currentSpeed;
...
<<<<<<< HEAD
            // Vector2 pos = predict * model.Output.timeResolution;
