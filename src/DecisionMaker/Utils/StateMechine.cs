...
class StateMachineNode<ActionType> : StateMachineNodeBase where ActionType : IAction, new()
{
...
class StateMachine
{
    ...
    public void FromTo(StateMachineNodeBase form, CheckFunction checkFunction, StateMachineNodeBase to)
    => form.checks.Add(() => { if (checkFunction()) return to; else return form; });
