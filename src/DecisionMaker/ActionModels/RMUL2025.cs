...
class RMUL2025DecisionMaker : Component, IPositionDecider
{
    ...
    public override sealed void Start()
    {
        ...
        var tracingEngineer = new BehaviourTreeAction(() =>
        {
            ...
            else TargetPosition = new(engineerAgent.Position.x, engineerAgent.Position.y, 0);
            return ActionState.Success;
        });
        ...
        var notFindHero = new BehaviourTreeCondition(() => !heroAgent.Found && (DateTime.Now - bb_hero_tracing_time).TotalSeconds >= 10);
        ...
        var findHero = new BehaviourTreeCondition(() => heroAgent.Found || (DateTime.Now - bb_hero_tracing_time).TotalSeconds < 10);
        ...
        var tracingHero = new BehaviourTreeAction(() =>
        {
            ...
            if (heroAgent.Locked && (sentry.Position.xy - heroAgent.Position).Length < 1)
                TargetPosition = sentry.Position;
            else TargetPosition = new(heroAgent.Position.x, heroAgent.Position.y, 0);
            return ActionState.Success;
        });
        var inFortress = new BehaviourTreeCondition(() => (sentry.Position - FortressPosition).Length < 0.5);
        ...
        var coreCharge = new StateMachineNode(() => { coreChargeBT.Action(); });
        var clashSurge = new StateMachineNode(() => { clashSurgeBT.Action(); });
        var supplySelf = new StateMachineNode(() => { TargetPosition = SupplyPosition; });
        ...
        stateMachine.FromTo(coreCharge,
        () => durationFromBegin.TotalMinutes > 2.5 && durationFromBegin.TotalMinutes <= 5.5,
        clashSurge);

        stateMachine.FromTo(coreCharge,
        () => durationFromBegin.TotalMinutes > 5.5 && 5 * (decisionMakingInfo.BulletCount + decisionMakingInfo.BulletSupplyCount) > decisionMakingInfo.EnemyBaseHp,
        headStrike);

        stateMachine.FromTo(clashSurge,
        () => durationFromBegin.TotalMinutes > 5.5 && 5 * (decisionMakingInfo.BulletCount + decisionMakingInfo.BulletSupplyCount) > decisionMakingInfo.EnemyBaseHp,
        headStrike);

        stateMachine.FromTo(supplySelf,
        () => durationFromBegin.TotalMinutes <= 2.5 && decisionMakingInfo.SentryHp > 350,
        coreCharge);

        stateMachine.FromTo(supplySelf,
        () => durationFromBegin.TotalMinutes > 2.5 && durationFromBegin.TotalMinutes <= 5.5 && decisionMakingInfo.SentryHp > 350,
        clashSurge);

        stateMachine.FromTo(supplySelf,
        () => durationFromBegin.TotalMinutes > 5.5 && decisionMakingInfo.SentryHp > 350 && decisionMakingInfo.BulletSupplyCount == 0,
        headStrike);

        stateMachine.FromTo(headStrike,
        () => decisionMakingInfo.BulletSupplyCount >= 200 && decisionMakingInfo.SentryHp < 300 && sentry.Position.x < 0,
        supplySelf);
