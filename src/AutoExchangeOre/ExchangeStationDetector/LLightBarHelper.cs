    ...
    public List<(Point pointInFrame, MCvPoint3D32f point3dInWorld)> UpdateKeypoints(Dictionary<int, (Rectangle box, List<Point> keypoints, float confidence, float keypointConfidenceSum)> detections)
    {
        ...
        foreach (var detection in detections)
        {
            ...
            if (classId < lLightBars.Length && classId >= 0)
            {
                ...
                {
                    updatedPoints.Add((new Point((int)keypoints[i].X, (int)keypoints[i].Y), lLightBars[classId].point3D[i]));
                }
            }
        }
        return updatedPoints;
    }
    public void DrawDetections(Mat image, Dictionary<int, (Rectangle box, List<Point> keypoints, float confidence, float keypointConfidenceSum)> detections)
    {
        ...
        foreach (var detection in detections)
        {
            ...
            {
                CvInvoke.Circle(image, new System.Drawing.Point((int)point.X, (int)point.Y), 5, new MCvScalar(255 - i * 40, i++ * 40, 0), -1);
            }
        }
    }
}
