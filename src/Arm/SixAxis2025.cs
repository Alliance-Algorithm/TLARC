...
class SixAxis : Component
{
  (Vector3d position, Quaterniond rotation) cameraInBase = (
    new(0.04915, -0.1057, 0.09735),
    Quaterniond.AxisAngleD(Quaterniond.AxisAngleD(Vector3d.AxisY, 45) * Vector3d.AxisZ, -90)
      * Quaterniond.AxisAngleD(Vector3d.AxisY, 45)
  );
  readonly double[] _pole = [0.05985, 0.41, 0.08307, 0.33969, 0.0571];
  RapidlyArmPlanner.ArmPlanner planner;

  [ComponentReferenceFiled]
  IRedemptionDetector redemptionDetector;
  ...
  public override void Start()
  {
    ...
    floatMultiArray.Subscript(
      "/engineer/joint/measure",
      x =>
      {
        for (int i = 0; i < 6; i++)
          joints[i] = x[i];
  ...
  public override void Update()
  {
    ...
    if (!isSuccess)
    {
      ...
      isSuccess = planner.PlanTrajectory(
        joints,
        (
          Quaterniond.AxisAngleR(Vector3d.AxisZ, joints[0])
            * (cameraInBase.position + cameraInBase.rotation * position),
          Quaterniond.AxisAngleR(Vector3d.AxisZ, joints[0]) * cameraInBase.rotation * rotation
        ),
        out var tmpTrajectory
      );
    ...
    floatMultiArray.Publish(
      trajectory
        .Select(x => (float)x.GetPosition((DateTime.Now - constructTime).TotalSeconds))
        .ToArray()
    );
  }
}
