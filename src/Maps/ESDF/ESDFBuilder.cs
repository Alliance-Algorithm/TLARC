  ...
  )
  {
    ...
    foreach (var _dynamicMap in dynamicMaps)
    {
      ...
      for (int j = 0; j < k; j++)
      {
        ...
        var x = (int)
          Math.Round(
            (i) * _dynamicMap.forward.Cos - (j) * _dynamicMap.forward.Sin + _dynamicMap.offsetX
          );
        var y = (int)
          Math.Round(
            (j) * _dynamicMap.forward.Cos + (i) * _dynamicMap.forward.Sin + _dynamicMap.offsetY
          );
        if (x < 0 || x >= sizeX)
          continue;
        if (y < 0 || y >= sizeY)
        ...
        _colored[x, y] = (sbyte)(
          (sbyte)(_colored[x, y] + (sbyte)(_map[x, y] / 4)) < _colored[x, y]
            ? sbyte.MaxValue
            : (_colored[x, y] + (sbyte)(_map[x, y] / 4))
        );
        _colored[x, y] = (sbyte)(
          (sbyte)(_colored[x, y] + (sbyte)(100 / max_queue_length + 1)) < _colored[x, y]
            ? sbyte.MaxValue
            : (_colored[x, y] + (sbyte)(100 / max_queue_length + 1))
        );
        if (_colored[x, y] >= 100)
        {
          _map[x, y] = 0;
    ...
    while (openList.Count > 0)
    {
      ...
      for (int j = -1; j <= 1; j++)
      {
        ...
        if (cObstacleX != -1)
        {
          var m = (sbyte)(
            Math.Clamp(
              Math.Round(Math.Sqrt(Math.Pow(cObstacleX - c.x, 2) + Math.Pow(cObstacleY - c.y, 2)))
                * staticMap.Resolution
                / maxDistance,
              0,
              1
            ) * 100
