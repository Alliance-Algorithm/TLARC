...
class BSplineTrajectoryWithMinimalSnap
{
    ...
    int lastIndex = -1;
    public static LinearConstraintCollection CreateConstraints(double[,] a, double[] b, LinkedList<(double value, double loose)> loose)
    {
    ...
    public BSplineTrajectoryWithMinimalSnap(LinkedList<(double value, double loose)> values)
    {
        ...
        // }
        var l = new double[1, 4] { { 0, 0, 0, 6 } }.Dot(M4S);
        ...
        QuadraticObjectiveFunction func = new QuadraticObjectiveFunction(H, new double[values.Count + 4 - 1]);
    ...
    public double GetPosition(double time)
    {
        ...
        // u = 0;
        return new double[] { 1, u, u * u, u * u * u }.Dot(M4Data).Dot(controlPoints.Get(k - 2, k + 2));
    }
    public static void ReAllocTimeline(List<BSplineTrajectoryWithMinimalSnap> lines)
    {
        ...
        while (true)
        {
            ...
            for (int k = 0; k < lines.Count; k++)
                ...
                {
                    tmpControlPoints[k, j] = 3 * (lines[k].controlPoints[j + 1] - lines[k].controlPoints[j]) / (timeline[j + 3] - timeline[j]);
                }
            ...
            for (int k = 2; k < timeline.Length - 3; k++)
            {
                ...
                var vs = tmpControlPoints.Get(0, 0, k - 2, k).Dot(new double[,] { { m_00 }, { m_01 } });
                foreach (var v in vs)
                    if (vMax < Math.Sqrt(v))
                    {
                        index = k;
        ...
        while (true)
        {
            ...
            for (int k = 0; k < lines.Count; k++)
                ...
                {
                    tmpControlPoints[k, j] = 2 * (tmpControlPoints[k, j + 1] - tmpControlPoints[k, j]) / (timeline[j + 2] - timeline[j]);
                }
