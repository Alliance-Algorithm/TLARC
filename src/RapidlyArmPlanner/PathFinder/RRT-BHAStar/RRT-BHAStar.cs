...
class RRT_BHAStar : IPathFinder<double>
{
  ...
  public RRT_BHAStar(double[] upperBound, double[] lowerBound)
  {
  ...
  public List<LinkedList<double>> Search(double[] start, double[] target)
  {
    ...
    while (true)
    {
      for (int i = 0; i < tailStartNode.Values.Length; i++)
        path[i].AddFirst(tailStartNode.Values[i]);
    ...
    while (true)
    {
      for (int i = 0; i < tailEndNode.Values.Length; i++)
        path[i].AddLast(tailEndNode.Values[i]);
