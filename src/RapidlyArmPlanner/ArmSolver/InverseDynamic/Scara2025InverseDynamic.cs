...
{
  public Scara2025InverseDynamic(
    double d1,
    double d2,
    double d3,
    (double mim, double max) limit1,
    (double mim, double max) limit2,
    (double mim, double max) limit3
  )
  {
    _d1 = d1;
    _d2 = d2;
  ...
  private readonly double _d1,
    _d2,
    _d3;
  private readonly (double min, double max) _l1,
    _l2,
    _l3;
  ...
  )
  {
    ...
    if (angle1 > _l1.min && angle1 < _l1.max && angle2 > _l2.min && angle2 < _l2.max && height > 0)
    {
      ...
      {
        thetas.Add(
          [angle1, height, angle2, RescaleToPi(-rollAngle), yawAngle, RescaleToPi(tailRollAngle)]
        );
        ret = true;
      }
      if (-yawAngle > _l3.min && -yawAngle < _l3.max)
  ...
  public bool Solve((Vector3d position, Quaterniond forward) target, out List<double[]> thetas)
  {
    ...
    ret =
      SolveTail(
        RescaleToPi(angle11 + angle12, [_l1.min, _l1.max]),
        -angle2,
        height,
        target,
        ref thetas
