...
class MPCController : Component
{
    ...
    Tlarc.IO.ROS2Msgs.Geometry.Pose2D velocityPub;
    Tlarc.IO.ROS2Msgs.Std.Bool spinningPub;
    ...
    public override void Start()
    {
        ...
        velocityPub = new(IOManager);
        spinningPub = new(IOManager);
    ...
    override public void Update()
    {
        ...
        if (!double.IsNaN(kesi))
            lastKesiAngle = kesi;
        b[2, 0] = t * Math.Tan(lastKesiAngle);
        b[2, 1] = t * targetVel.Length() / Math.Pow(Math.Cos(lastKesiAngle), 2);
        ...
        lastAngle = tempAngle;
        // Vector2 targetTmp = new(targetVel.X, targetVel.Y);
        // vel = (vel + targetTmp) / 2;
