...
using Tlarc.IO.ROS2Msgs.Geometry;
...
class HybridAStar : Component
{
    ...
    public override void Update()
    {
        ...
        var vel = sentry.position - _agentPosition;
        _beginSpeedAngle = MathF.Atan2(vel.Y, vel.X);
        if (vel.Length() <= 0.1f)
            _beginSpeedAngle = float.PositiveInfinity;
        if (costMap[x, y] <= 0)
    ...
    private bool Search()
    {
        var maxDistance = maxSearchDistance;
        _path = [];
        openList = new();
        ...
        while (openList.Count > 0)
        {
            ...
            var children = current.ChildrenGen(costMap);
