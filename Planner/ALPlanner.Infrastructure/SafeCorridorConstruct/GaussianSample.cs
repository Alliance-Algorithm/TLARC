    ...
    public static ISafeCorridor2DData<Circle2D> RadiusWithDistance(IPath2D astarPath, IObstacle obstacle)
    {
        ...
        foreach (var path in astarPath.GetPoints())
        {
            ...
            corridor.AddLast((from offset in GaussianOffset
                              ...
                              select new Circle2D(Math.Clamp(dis, (path + offset - corridor.Last!.Value.Origin).Length() - corridor.Last!.Value.R, (path + offset - corridor.Last!.Value.Origin).Length()), path + offset))
                              .MaxBy(item => -(item.Origin - corridor.Last!.Value.Origin).Length() + item.R)!);
