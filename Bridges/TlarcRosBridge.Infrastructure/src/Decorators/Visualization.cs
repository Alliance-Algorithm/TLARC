...
internal static class Visualization
{
    ...
    static void Draw(in AABB2D circle, ref Messages.Visualization.Marker.Priv marker)
    {
        marker.Type = Messages.Visualization.Marker.CUBE;
        marker.Scale.X = circle.MaxX - circle.MinX;
        marker.Scale.Y = circle.MaxY - circle.MinY;
        marker.Scale.Z = 0.1;

        marker.Pose.Position.X = (circle.MinX + circle.MaxX) / 2;
        marker.Pose.Position.Y = (circle.MinY + circle.MaxY) / 2;
        marker.Pose.Position.Z = 0;
    }
    static internal void Draw(in string id, IRclNode node,
    in SafeCorridor2DData<AABB2D> rectangle,
    ref Messages.Visualization.MarkerArray.Priv markers)
    {
        markers.Markers = new(rectangle.Length);
        var span = markers.Markers.AsSpan();
